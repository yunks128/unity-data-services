resource "aws_api_gateway_resource" "misc_proxy_resource" {
  rest_api_id = data.aws_api_gateway_rest_api.rest_api.id
  parent_id   = aws_api_gateway_resource.misc_base_resource.id
  path_part   = "{proxy+}"
}
module "misc_proxy_any_to_lambda_module" {
  ...
  rest_api_id      = data.aws_api_gateway_rest_api.rest_api.id
  resource_id      = aws_api_gateway_resource.misc_proxy_resource.id
}


module "misc_proxy_cors_method" {
  source           = "./cors_module"
  rest_api_id      = data.aws_api_gateway_rest_api.rest_api.id
  resource_id      = aws_api_gateway_resource.misc_proxy_resource.id
  prefix           = "${var.prefix}_misc_proxy"
}