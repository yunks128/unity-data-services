    ...
    def test_02_download(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            for each_granule in zip(granule_json['features'], download_result):
                ...
                self.assertEqual(each_granule[1]['assets']['data']['href'], os.path.join('.', remote_filename), f"mismatched: {each_granule[0]['assets']['data']['href']}")
            self.assertTrue(FileUtils.file_exist(os.environ['OUTPUT_FILE']), f'missing output file')
        return
    ...
    def test_02_download__daac(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            for each_granule in zip(granule_json['features'], download_result):
                ...
                self.assertEqual(each_granule[1]['assets']['metadata']['href'], os.path.join('.', remote_filename), f"mismatched: {each_granule[0]['assets']['metadata']['href']}")
            self.assertTrue(FileUtils.file_exist(os.environ['OUTPUT_FILE']), f'missing output file')
        return
