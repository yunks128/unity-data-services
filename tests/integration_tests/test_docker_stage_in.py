...
class TestDockerStageIn(TestCase):
    ...
    def test_02_download__from_dapa_url(self):
        granule_json = '{"numberMatched": {"total_size": 5}, "numberReturned": 6, "stac_version": "1.0.0", "type": "FeatureCollection", ' \
                       ...
                       ']}'
        granule_json = json.loads(granule_json)
        dapa_url = 'https://1gp9st60gd.execute-api.us-west-2.amazonaws.com/dev/sbx-uds-2-dapa/collections/URN:NASA:UNITY:UDS_LOCAL_TEST:DEV:UDS_COLLECTION___2312041030/items?limit=37&offset=0&datetime=1990-01-14T08:00:00Z/2024-01-14T11:59:59Z'
        if len(argv) > 1:
            argv.pop(-1)
        argv.append('DOWNLOAD')
